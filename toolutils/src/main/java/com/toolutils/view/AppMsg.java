...
public class AppMsg {
    ...
    private  static ListView tv;
    private static AppMsgListAdapter appMsgListAdapter;
    private static List<String> msglst=new ArrayList<>();
    private static TimerTask timerTask;
    private static Timer timer;
    /**
     *
     * @author mengguoqiang 直持多条提示弹出，按顺序显示
     * Make a {@link AppMsg} with a custom view. It can be used to create non-floating notifications if floating is false.
     *
     * @param context  The context to use. Usually your
     *                 {@link Activity} object.
     ...
     * @param floating true if it'll float.
     */
    private static AppMsg makeText(final Activity context, List<String> text, Style style, View view, boolean floating, float textSize) {
        AppMsg result = new AppMsg(context);

        view.setBackgroundResource(style.background);
        tv = (ListView) view.findViewById(R.id.lv_msg_lst);
        msglst.clear();
        msglst.addAll(text);
        List<String> tmp = new ArrayList<>();
        tmp.add(msglst.get(0));
        appMsgListAdapter = new AppMsgListAdapter(context, text);
        tv.setAdapter(appMsgListAdapter);
        result.mView = view;
        result.mDuration = style.duration;
        result.mFloating = floating;
        return result;
     ...
     * @param style    The style with a background and a duration.
     */
    public static AppMsg makeText(Activity context, List<String> text, Style style, View customView) {
        return makeText(context, text, style, customView, false);
    }
     ...
     * @param floating true if it'll float.
     */
    private static AppMsg makeText(Activity context, List<String> text, Style style, View view, boolean floating) {
        return makeText(context, text, style, view, floating, 0);
    }
     ...
     * @param style   The style with a background and a duration.
     */
    public static AppMsg makeText(Activity context, List<String> text, Style style) {
        return makeText(context, text, style, R.layout.app_msg_list);
    }
     ...
     * @param style   The style with a background and a duration.
     */
    public static AppMsg makeText(Activity context, List<String> text, Style style, int layoutId) {
        LayoutInflater inflate = (LayoutInflater)
                context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View v = inflate.inflate(layoutId, null);
        return makeText(context, text, style, v, true);
    ...
    //显示列表
    public static void setAppMsgList(List<String> msglst)
    {
        appMsgListAdapter.setMsglst(msglst);
    }
}
